...
class OgnROS2RtxLidarHInternalState(BaseWriterNode):
    def __init__(self):
        self.viewport = None
        self.viewport_name = ""
    ...
    def post_attach(self, writer, render_product):
        try:
            omni.syntheticdata.SyntheticData.Get().set_node_attributes(
                ...
                {"inputs:resetOnStop": self.resetSimulationTimeOnStop},
                render_product,
            )
        except:
            pass
...
class OgnROS2RtxLidarH:
    @staticmethod
    def internal_state():
        return OgnROS2RtxLidarHInternalState()
        ...
        if db.internal_state.initialized is False:
            ...
            with Usd.EditContext(stage, stage.GetSessionLayer()):
                ...
                if stage.GetPrimAtPath(render_product_path) is None:
                    ...
                    carb.log_warn(
                        "Render product not created yet, retrying on next call"
                    )
                    db.internal_state.initialized = False
                    return False
                else:
                    prim = stage.GetPrimAtPath(
                        get_camera_prim_path(render_product_path)
                    )
                ...
                if db.internal_state.sensor is None:
                    carb.log_warn(
                        "Active camera for Render product is not an RTX Lidar"
                    )
                    db.internal_state.initialized = False
                    return False
                ...
                db.internal_state.resetSimulationTimeOnStop = (
                    db.inputs.resetSimulationTimeOnStop
                )
                writer = None
                try:
                    if sensor_type == "laser_scan":
                        if db.inputs.fullScan:
                            carb.log_warn(
                                "fullScan does not have an effect with the laser_scan setting"
                            )
                        ...
                        if db.inputs.fullScan:
                            ...
                            writer = rep.writers.get(
                                "RtxLidar" + "ROS2PublishPointCloud"
                            )
    ...
    def release(node):
        try:
            state = OgnROS2RtxLidarHInternalState.per_node_internal_state(node)
        except Exception:
            state = None
            pass
